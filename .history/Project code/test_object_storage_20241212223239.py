...
def test_write_table_to_s3_csv(s3_bucket):
    ...
    assert result_df.iloc[0]['shikimori_username'] == 'test_user'
    assert result_df.iloc[0]['last_updated'] == get_old_timestamp().isoformat()
...
def test_load_user_anime_list(s3_bucket):
    load_user_anime_list(1, [{'anime': {'id': 2, 'name': 'One Piece', 'score': 9, 'genres': [{'name': 'Adventure'}], 'episodes': 24, 'poster': {'originalUrl': 'http://example.com'}}, 'status': 'finished', 'score': 9}])
