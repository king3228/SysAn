...
def s3_bucket():
    with mock_aws():
        client = boto3.client('s3', region_name='ru-central1')
        client.create_bucket(Bucket=BUCKET_NAME)
        
        # Создаем тестовые dataframes и заливаем на S3
...
def test_get_old_timestamp():
    assert get_old_timestamp() == datetime(1970, 1, 1, tzinfo=datetime.timezone.utc)
