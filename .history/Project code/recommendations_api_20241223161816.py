import requests
import json
from shiki_api import get_anime_by_id
...
def get_detailed_single_list(list_id):
    ...
    for anime_id in list_data["anime_ids"]:
        ...
        details = get_anime_by_id(anime_id)
        anime_titles.append(f"{details['title']} - {details['url']}")
...
def compare_list_with_watched(list_id, chat_id):
    ...
    if response.status_code == 200:
        ...
        result = {
            "viewed": [get_anime_by_id(anime_id)["title"] for anime_id in viewed],
            "not_viewed": [get_anime_by_id(anime_id)["title"] for anime_id in not_viewed]
        }
        return result
    else:
        return handle_error(response)