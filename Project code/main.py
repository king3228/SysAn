...
from db_controller import (connect_db, check_user_exists, add_user,
                           update_last_updated, add_shiki_to_user,
                           load_user_anime_list, get_context_data)
...
def handle_start_command(chat_id):
    driver = connect_db()
    user_info = check_user_exists(driver, chat_id)
        ...
        if user_info:
            ...
            if (seconds_since_epoch - last_updated) > 86400:
                if shikimori_username:
                    ...
                    load_user_anime_list(driver, chat_id, user_anime_list)
                    update_last_updated(driver, chat_id)
                else:
                    send_message(chat_id, "Необходимо привязать аккаунт Shikimori, чтобы обновить список.")
            
            context_data = get_context_data(driver, chat_id)
...
def handle_message(chat_id, text):
    ...
    if chat_id in user_state:
        if user_state[chat_id] == 'awaiting_id':
            add_shiki_to_user(driver, chat_id, text)
